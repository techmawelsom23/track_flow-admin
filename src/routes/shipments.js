...
router.post('/from-inquiry', authMiddleware, adminOnly, async (req, res) => {
  ...
  if (!iq.rows[0].quote_cost_usd) return res.status(400).json({ error: 'This inquiry has not been quoted yet' });
...
router.post('/update-location', authMiddleware, adminOnly, async (req, res) => {
